// PSGC data for demonstration. In a real app, this would be a comprehensive database or API.
export const PSGC_DATA = {
  provinces: [
    { name: 'Abra', code: '140100000' },
    { name: 'Agusan del Norte', code: '160200000' },
    { name: 'Agusan del Sur', code: '160300000' },
    { name: 'Aklan', code: '060400000' },
    { name: 'Albay', code: '050500000' },
    { name: 'Antique', code: '060600000' },
    { name: 'Apayao', code: '148100000' },
    { name: 'Aurora', code: '037700000' },
    { name: 'Basilan', code: '150700000' },
    { name: 'Bataan', code: '030800000' },
    { name: 'Batanes', code: '020900000' },
    { name: 'Batangas', code: '041000000' },
    { name: 'Benguet', code: '141100000' },
    { name: 'Biliran', code: '087800000' },
    { name: 'Bohol', code: '071200000' },
    { name: 'Bukidnon', code: '101300000' },
    { name: 'Bulacan', code: '031400000' },
    { name: 'Cagayan', code: '021500000' },
    { name: 'Camarines Norte', code: '051600000' },
    { name: 'Camarines Sur', code: '051700000' },
    { name: 'Camiguin', code: '101800000' },
    { name: 'Capiz', code: '061900000' },
    { name: 'Catanduanes', code: '052000000' },
    { name: 'Cavite', code: '042100000' },
    { name: 'Cebu', code: '072200000' },
    { name: 'Cotabato', code: '124700000' },
    { name: 'Davao de Oro', code: '118200000' },
    { name: 'Davao del Norte', code: '112300000' },
    { name: 'Davao del Sur', code: '112400000' },
    { name: 'Davao Occidental', code: '118600000' },
    { name: 'Davao Oriental', code: '112500000' },
    { name: 'Dinagat Islands', code: '168500000' },
    { name: 'Eastern Samar', code: '082600000' },
    { name: 'Guimaras', code: '067900000' },
    { name: 'Ifugao', code: '142700000' },
    { name: 'Ilocos Norte', code: '012800000' },
    { name: 'Ilocos Sur', code: '012900000' },
    { name: 'Iloilo', code: '063000000' },
    { name: 'Isabela', code: '023100000' },
    { name: 'Kalinga', code: '143200000' },
    { name: 'La Union', code: '013300000' },
    { name: 'Laguna', code: '043400000' },
    { name: 'Lanao del Norte', code: '103500000' },
    { name: 'Lanao del Sur', code: '153600000' },
    { name: 'Leyte', code: '083700000' },
    { name: 'Maguindanao del Norte', code: '158400000' },
    { name: 'Maguindanao del Sur', code: '158800000' },
    { name: 'Marinduque', code: '174000000' },
    { name: 'Masbate', code: '054100000' },
    { name: 'Metro Manila', code: '130000000' },
    { name: 'Misamis Occidental', code: '104200000' },
    { name: 'Misamis Oriental', code: '104300000' },
    { name: 'Mountain Province', code: '144400000' },
    { name: 'Negros Occidental', code: '064500000' },
    { name: 'Negros Oriental', code: '074600000' },
    { name: 'Northern Samar', code: '084800000' },
    { name: 'Nueva Ecija', code: '034900000' },
    { name: 'Nueva Vizcaya', code: '025000000' },
    { name: 'Occidental Mindoro', code: '175100000' },
    { name: 'Oriental Mindoro', code: '175200000' },
    { name: 'Palawan', code: '175300000' },
    { name: 'Pampanga', code: '035400000' },
    { name: 'Pangasinan', code: '015500000' },
    { name: 'Quezon', code: '045600000' },
    { name: 'Quirino', code: '025700000' },
    { name: 'Rizal', code: '045800000' },
    { name: 'Romblon', code: '175900000' },
    { name: 'Samar', code: '086000000' },
    { name: 'Sarangani', code: '128000000' },
    { name: 'Siquijor', code: '078900000' },
    { name: 'Sorsogon', code: '056200000' },
    { name: 'South Cotabato', code: '126300000' },
    { name: 'Southern Leyte', code: '086400000' },
    { name: 'Sultan Kudarat', code: '126500000' },
    { name: 'Sulu', code: '156600000' },
    { name: 'Surigao del Norte', code: '166700000' },
    { name: 'Surigao del Sur', code: '166800000' },
    { name: 'Tarlac', code: '036900000' },
    { name: 'Tawi-Tawi', code: '157000000' },
    { name: 'Zambales', code: '037100000' },
    { name: 'Zamboanga del Norte', code: '097200000' },
    { name: 'Zamboanga del Sur', code: '097300000' },
    { name: 'Zamboanga Sibugay', code: '098300000' },
  ].sort((a, b) => a.name.localeCompare(b.name)), // Keep the list sorted alphabetically
  cities: {
    // Sample data for Cebu
    '072200000': [ 
      { name: 'Cebu City', code: '072217000' },
      { name: 'Mandaue City', code: '072230000' },
    ],
    // Sample data for Metro Manila
    '130000000': [ 
      { name: 'Makati City', code: '137602000' },
      { name: 'Quezon City', code: '137404000' },
    ],
    // NOTE: A full list of cities for all provinces is too large to include here.
    // In a production application, this data should be fetched from a database or API.
  },
  barangays: {
     // Sample data for Cebu City
    '072217000': [
      { name: 'Lahug', code: '072217020' },
      { name: 'Guadalupe', code: '072217017' },
    ],
     // Sample data for Mandaue City
    '072230000': [
      { name: 'Subangdaku', code: '072230024' },
      { name: 'Banilad', code: '072230005' },
    ],
    // Sample data for Makati City
    '137602000': [ 
      { name: 'Poblacion', code: '137602014' },
      { name: 'Bel-Air', code: '137602002' },
    ],
    // Sample data for Quezon City
    '137404000': [ 
      { name: 'Diliman', code: '137404021' },
      { name: 'Batasan Hills', code: '137404009' },
    ],
    // NOTE: A full list of barangays for all cities is too large to include here.
    // In a production application, this data should be fetched from a database or API.
  },
};

export type Province = (typeof PSGC_DATA.provinces)[0];
export type City = (typeof PSGC_DATA.cities)['072200000'][0];
export type Barangay = (typeof PSGC_DATA.barangays)['072217000'][0];
