...
/**
 * ProfileV2 Page
 * Dedicated route for the high-fidelity ProfileV2 experience.
 */
export default function ProfileV2Page() {
  const account = useCurrentAccount();
  const [characters, setCharacters] = useState<any[]>([]);
  const [index, setIndex] = useState(0);
  const [activeTab, setActiveTab] = useState<"Collections" | "Orders" | "Stats" | "Badges">("Stats");
      ...
      try {
        ...
        const parsed = (ownedObjects || [])
          .map((obj: any) => {
            ...
            // Try multiple fallback fields for the image
            const imgUrl = displayData.image_url || displayData.image || contentData.image_url || contentData.url || "";

            return {
              objectId: obj.data?.objectId || "",
              name: displayData.name || contentData.name || "Unnamed Spirit",
              description: displayData.description || contentData.description || "A mysterious spirit from the Kapogian realm.",
