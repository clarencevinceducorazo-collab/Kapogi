...
// ── Robust URL resolver ────────────────────────────────────────────────────────
// Tries multiple field paths and IPFS formats to find a valid image URL.
function resolveImageUrl(obj: any): string {
  const displayData  = obj.data?.display?.data  ?? {};
  const contentData  = obj.data?.content?.fields ?? {};

  // Priority order: display.image_url → content.url → content.image_url → content.image
  const rawUrl =
  ...
  if (!rawUrl) return "/images/KapogianLogo.webp"; // fallback placeholder

  // Already an https URL — pass through getIPFSGatewayUrl which handles both
  return getIPFSGatewayUrl(rawUrl);
}

// ── Robust attributes parser ───────────────────────────────────────────────────
// Handles: JSON string, plain object, or undefined/null
function parseAttributes(contentData: any): Record<string, any> {
  const raw = contentData.attributes;
  if (!raw) return {};
...
const StatDisplay = ({
  ...
  <div className="space-y-1">
    ...
    <Progress
      value={value}
      className="h-3 border-2 border-black bg-white/50 [&>div]:bg-primary"
    />
  </div>
);
...
export default function CollectionPage() {
  ...
  const loadCharacters = async () => {
    ...
    try {
      ...
      // DEBUG: log raw response in development to help diagnose field issues
      if (process.env.NODE_ENV === "development") {
        console.log("[Collection] raw objects:", ownedObjects.slice(0, 2));
      }
      ...
      const parsedCharacters: Character[] = ownedObjects
        .map((obj: any) => {
          const displayData = obj.data?.display?.data  ?? {};
          const contentData = obj.data?.content?.fields ?? {};

          const attributes = parseAttributes(contentData);
          const imageUrl   = resolveImageUrl(obj);

          // Name: try display first, then content fields, then fallback
          const name =
            displayData.name        ||
            contentData.name        ||
            contentData.character_name ||
            "Unnamed Kapogian";
          ...
          return {
            ...
            imageUrl,
            attributes: {
              cuteness:    Number(attributes.cuteness    ?? attributes.Cuteness    ?? 0),
              confidence:  Number(attributes.confidence  ?? attributes.Confidence  ?? 0),
              tiliFactor:  Number(attributes.tiliFactor  ?? attributes.tili_factor ?? attributes.TiliFactor ?? 0),
              ...attributes,
            },
          };
        })
      ...
      setSelectedChar(parsedCharacters[0] ?? null);
    } catch (err: any) {
      console.error("[Collection] loadCharacters error:", err);
      setError("Failed to load your collection. Please try refreshing.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-6 pt-32 pb-20 relative z-10">
          ...
          {!account ? (
            <div className="flex items-center justify-center min-h-[60vh]">
              <div className="bg-white border-4 border-black rounded-3xl p-12 shadow-hard text-center max-w-md w-full">
                ...
                <h2 className="font-headline text-3xl mb-4 text-black uppercase">
                  Sync Required
                </h2>
          ...
          /* ── Loading ── */
            ...
            <div className="h-[50vh] flex flex-col items-center justify-center gap-4 bg-white border-4 border-black rounded-3xl shadow-hard">
              ...
              <p className="font-black uppercase">Syncing your squad...</p>
            </div>

          /* ── Error ── */
          ) : error ? (
            <div className="h-[50vh] flex flex-col items-center justify-center gap-6 bg-white border-4 border-black rounded-3xl shadow-hard text-center px-8">
              <PackageOpen size={48} className="text-red-400" />
              <p className="font-black text-red-500 uppercase">{error}</p>
              <Button
                onClick={loadCharacters}
                className="bg-black text-white font-black rounded-xl px-8 py-4 border-2 border-black shadow-hard-sm hover:bg-primary"
              >
                Try Again
              </Button>
            </div>
          ...
          /* ── No NFTs ── */
            ...
            <div className="h-[50vh] flex flex-col items-center justify-center gap-6 bg-white border-4 border-black rounded-3xl shadow-hard text-center px-8">
              ...
              <div>
                <h2 className="font-headline text-3xl text-black uppercase mb-2">No Kapogians Found</h2>
                <p className="text-gray-500 font-semibold">This wallet has no Kapogian Spirit NFTs yet.</p>
              </div>
              <a href="/generate">
                <Button className="bg-yellow-400 hover:bg-accent text-black font-black rounded-xl px-8 py-4 border-4 border-black shadow-hard-sm">
                  <Sparkles size={18} className="mr-2" /> Summon Your First Kapogian
                </Button>
              </a>
            </div>
                ...
                <div className="bg-white border-4 border-black rounded-2xl px-8 py-5 shadow-hard flex items-center gap-6">
                  <div className="text-left">
                    ...
                    <span className="text-3xl font-black text-black">
                      {characters.length}{" "}
                      <span className="text-sm font-bold text-gray-500 uppercase">Squad</span>
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
                ...
                <div className="lg:col-span-8">
                  {selectedChar ? (
                    <div className="bg-white border-4 border-black rounded-3xl overflow-hidden shadow-hard p-8 h-full flex flex-col">
                      <div className="flex flex-col md:flex-row gap-10 h-full">
                        <div className="md:w-1/2 aspect-square relative rounded-2xl overflow-hidden border-4 border-black bg-gray-50">
                          {/* Use regular img tag as fallback if Next Image domain not configured */}
                          <img
                            src={selectedChar.imageUrl}
                            alt={selectedChar.name}
                            className="w-full h-full object-cover"
                            onError={(e) => {
                              (e.target as HTMLImageElement).src = "/images/KapogianLogo.webp";
                            }}
                          />
                        </div>
                        <div className="md:w-1/2 flex flex-col justify-between">
                          <div>
                            ...
                            <h2 className="font-headline text-6xl uppercase mt-2 text-black leading-tight break-words">
                              {selectedChar.name}
                            </h2>
                          ...
                          <a
                            href={`https://suiexplorer.com/object/${selectedChar.objectId}`}
                            target="_blank"
                    ...
                    <div className="bg-white border-4 border-black rounded-3xl p-8 h-full flex items-center justify-center">
                      <p className="font-black text-gray-300 uppercase">Select a character</p>
                    </div>
                  )}
                </div>
                ...
                <div className="lg:col-span-4 flex flex-col gap-6">
                  <div className="bg-white border-4 border-black rounded-3xl p-6 shadow-hard flex flex-col flex-grow">
                    <div className="flex items-center justify-between mb-6">
                      <h3
                        className="font-headline text-2xl uppercase text-black"
                        style={{ textShadow: "1px 1px 0px #fff, 3px 3px 0px #000" }}
                      >
                        Squad Archive
                      </h3>
                      <div className="flex gap-2">
                        <button
                          ...
                          className="p-2 bg-white border-2 border-black rounded-lg shadow-hard-sm hover:bg-yellow-400 disabled:opacity-30 transition-brutal"
                        >
                          <ChevronLeft size={20} />
                        </button>
                        <button
                      ...
                      {displayedCharacters.map((char) => (
                        <button
                          ...
                          className={`relative aspect-square rounded-2xl overflow-hidden border-4 transition-brutal shadow-hard-sm bg-white ${
                            selectedChar?.objectId === char.objectId
                              ? "border-primary -translate-y-1 shadow-hard"
                              : "border-black hover:border-accent hover:-translate-y-1"
                          }`}
                        >
                          <img
                            src={char.imageUrl}
