...
export default function KapogianStoragePage() {
    ...
    try {
      ...
      if (data.files) {
        setFiles(data.files.map((f: any) => ({
          id: f.ipfsHash, name: f.name, cid: f.ipfsHash,
          size: f.size || 0, date: f.date || new Date().toISOString(),
          vis: 'public', group: f.group || '', url: f.url,
        })));
      }
    } catch (err) {
      console.error(err);
    ...
    try {
      ...
      if (data.data) {
        ...
        setGroups(list.map((g: any) => ({
          id: g.id, name: g.name, emoji: emojiMap[g.id] || '📁',
          date: g.createdAt || g.created_at || new Date().toISOString(), vis: 'public',
        })));
      }
    } catch (err) {
      console.error(err);
  ...
  const saveGroup = async () => {
    ...
    setIsLoadingGroups(true);
    try {
      const res  = await fetch('/api/pinata/groups', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newGroupName.trim() }) });
      const data = await res.json();
      ...
      setGroups(prev => [{ id: realId, name: data.data.name, emoji: newGroupEmoji, date: data.data.createdAt || new Date().toISOString(), vis: 'public' }, ...prev]);
      setNewGroupName('');
      setGroupModalOpen(false);
      addToast(`Group "${data.data.name}" created!`, 'success');
    } catch (err: any) {
  ...
  const copyText = (text: string, label = 'Value') => {
  ...
  const filteredFiles = useMemo(() => {
    ...
    return files
      ...
      .sort((a, b) => {
        let va: any = sk === 'size' ? a.size : (a as any)[sk === 'date' ? 'date' : sk];
        let vb: any = sk === 'size' ? b.size : (b as any)[sk === 'date' ? 'date' : sk];
  ...
  const filteredGroups = useMemo(() => {
    const q = searchQuery.toLowerCase();
    return groups.filter(g => g.vis === activeTab).filter(g => !q || g.name.toLowerCase().includes(q) || g.id.toLowerCase().includes(q));
  }, [groups, activeTab, searchQuery]);

  const totalPages   = Math.max(1, Math.ceil(filteredFiles.length / rowsPerPage));
  const currentFiles = useMemo(() => filteredFiles.slice((currentPage - 1) * rowsPerPage, currentPage * rowsPerPage), [filteredFiles, currentPage, rowsPerPage]);
  ...
  const deleteFile = async (id: string) => {
    try {
      const res = await fetch('/api/pinata/unpin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ hash: id }) });
  ...
  if (!unlocked) return <StorageGate onUnlocked={() => setUnlocked(true)} />;

  return (
    <div className="bg-gradient-to-br from-sky-50 via-white to-amber-50 text-slate-700 min-h-screen selection:bg-sky-200 selection:text-blue-900">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .neo{background:rgba(255,255,255,.7);backdrop-filter:blur(16px);border:2px solid rgba(255,255,255,.9);box-shadow:0 10px 30px rgba(186,230,253,.4)}
        .neo-sm{background:rgba(255,255,255,.8);backdrop-filter:blur(12px);border:2px solid rgba(255,255,255,.95);box-shadow:0 4px 15px rgba(186,230,253,.25)}
        .sidebar{background:rgba(255,255,255,.85);backdrop-filter:blur(24px);border-right:2px solid rgba(255,255,255,.9);box-shadow:4px 0 24px rgba(186,230,253,.3);z-index:40}
        .file-card{background:linear-gradient(145deg,rgba(255,255,255,.95),rgba(240,249,255,.8));backdrop-filter:blur(12px);border:3px solid #fff;transition:all .35s cubic-bezier(.34,1.56,.64,1);animation:card-in .4s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 8px 24px rgba(186,230,253,.25);border-radius:2.5rem}
        .file-card:hover{transform:translateY(-10px) scale(1.02);border-color:rgba(56,189,248,.4);box-shadow:0 20px 40px -5px rgba(56,189,248,.35)}
        ...
        .nav-item.active{background:linear-gradient(135deg,rgba(56,189,248,.15),rgba(251,191,36,.1));color:#0284c7;border-color:rgba(56,189,248,.3);box-shadow:0 4px 15px rgba(56,189,248,.15)}
        ...
        .modal-box{background:rgba(255,255,255,.95);backdrop-filter:blur(24px);border:4px solid #fff;box-shadow:0 25px 50px -12px rgba(14,165,233,.3)}
      ...
      <div className="relative z-10 flex min-h-screen pt-20">
        {/* Sidebar */}
        <aside className="sidebar fixed top-20 left-0 h-[calc(100vh-5rem)] w-52 z-40 flex-col py-7 px-3 hidden lg:flex">
          <div className="flex items-center gap-2.5 px-3 mb-7">
            <img src="/images/KapogianLogo.webp" alt="Kapogian" className="w-8 h-8 object-contain" />
            <div>
          ...
          <nav className="flex-1 space-y-1">
            <button onClick={() => setPage('files')} className={cn('w-full nav-item flex items-center gap-3 px-4 py-2.5 rounded-2xl font-black text-xs', page === 'files' ? 'active' : 'text-slate-400 hover:bg-slate-50')}>
              <FileIcon className={cn('w-4 h-4', page === 'files' ? 'text-sky-600' : 'text-slate-400')} /> Files
            </button>
            <button onClick={() => setPage('groups')} className={cn('w-full nav-item flex items-center gap-3 px-4 py-2.5 rounded-2xl font-black text-xs', page === 'groups' ? 'active' : 'text-slate-400 hover:bg-slate-50')}>
              <FolderOpen className={cn('w-4 h-4', page === 'groups' ? 'text-sky-600' : 'text-slate-400')} /> Groups
          ...
          <div className="max-w-6xl mx-auto px-5 pt-10 pb-28">
            ...
            <div className="flex items-center justify-between mb-7 flex-wrap gap-4 pl-12 lg:pl-0">
              <div className="flex items-center gap-4">
                ...
                {page === 'files' && groupFilter && (
                  <div className="flex items-center gap-2 bg-sky-50 border border-sky-100 px-3 py-1.5 rounded-2xl shadow-sm">
                    ...
                    <span className="text-xs font-black text-sky-400 uppercase italic">{groups.find(g => g.id === groupFilter)?.name || 'Selected'}</span>
                    <button onClick={() => setGroupFilter(null)} className="w-5 h-5 flex items-center justify-center rounded-full bg-sky-100 text-sky-500 hover:bg-red-500 hover:text-white transition-all"><X size={10} /></button>
                  </div>
                )}
              </div>
              <div className="flex gap-2">
                ...
                <button onClick={() => page === 'files' ? setUploadModalOpen(true) : setGroupModalOpen(true)} className="squishy bg-sky-500 text-white font-black px-6 py-3 rounded-full text-xs flex items-center gap-2 hover:bg-sky-400 transition-all uppercase tracking-widest shadow-[6px_6px_0_0_#bae6fd]">
                  <Plus size={18} /> Add
                </button>
              </div>
            </div>
            ...
            <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <p className="text-sm font-medium text-slate-400">
                {activeTab === 'public' ? 'Public assets permanently pinned via Pinata.' : 'Private assets for administrative access.'}
              </p>
              <div className="relative w-full max-w-sm">
                <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <input type="text" placeholder="Search IPFS manifests…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  className="w-full bg-white border border-slate-100 rounded-xl pl-10 pr-4 py-2 text-sm font-medium focus:border-sky-300 outline-none transition-all shadow-sm" />
              </div>
            </div>
              ...
              <div className="animate-in fade-in duration-500">
                {isLoadingFiles ? (
                  ...
                  <div className="text-center py-32 opacity-40">
                    <Database size={64} className="mx-auto mb-4 text-slate-400" />
                      ...
                      {currentFiles.map(file => (
                        <div key={file.id} className="file-card group rounded-[2.5rem] overflow-hidden">
                          <div className={cn('relative h-48 flex items-center justify-center overflow-hidden', thCls(file.name))}>
                            ...
                            <button
                              onClick={() => setDeleteConfirm({ fileId: file.id, fileName: file.name, type: 'file' })}
                              className="absolute top-4 right-4 w-8 h-8 bg-red-500 text-white rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-[6px_6px_0_0_rgba(0,0,0,0.2)]"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
                    ...
                    <div className="mt-12 flex justify-center">
                      <div className="neo-sm rounded-2xl px-6 py-3 border-2 border-white flex items-center gap-6">
                        ...
                        <div className="flex gap-4 items-center">
                          <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="squishy w-10 h-10 neo-sm rounded-xl flex items-center justify-center text-slate-400 hover:text-sky-500 disabled:opacity-20 border-2 border-white"><ArrowLeft size={18} /></button>
                          <span className="text-sm font-black text-slate-700 font-mono">{currentPage} / {totalPages}</span>
                          <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="squishy w-10 h-10 neo-sm rounded-xl flex items-center justify-center text-slate-400 hover:text-sky-500 disabled:opacity-20 border-2 border-white"><ArrowRight size={18} /></button>
                        </div>
                      </div>
                    </div>
                  </>
                          ...
                          <tr key={group.id} className="hover:bg-slate-50 transition-colors">
                            <td className="px-8 py-5">
                              <button onClick={() => navigateToGroup(group.id)} className="flex items-center gap-3 hover:translate-x-1 transition-transform group/btn">
                                ...
                                <span className="font-black text-slate-700 uppercase italic tracking-tighter underline decoration-2 decoration-sky-100 group-hover/btn:text-sky-500">{group.name}</span>
                              </button>
                            </td>
                            <td className="px-8 py-5 text-slate-400 font-bold text-sm">{fmtDt(group.date)}</td>
                            <td className="px-8 py-5">
                              <div onClick={() => copyText(group.id, 'Group ID')} className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-1.5 cursor-pointer w-fit hover:bg-sky-50 transition-colors">
                            ...
                            <td className="px-8 py-5">
                              <div className="flex items-center justify-center">
                                {/* Group delete → opens confirmation modal */}
                                <button
                                  onClick={() => setDeleteConfirm({ fileId: group.id, fileName: group.name, type: 'group' })}
                                  className="w-9 h-9 rounded-xl bg-red-50 text-red-400 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-sm"
                                >
                                  <Trash2 size={16} />
                                </button>
                              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="modal-box relative bg-white border-2 border-white rounded-[3rem] p-8 max-w-lg w-full shadow-[12px_12px_0_0_#7dd3fc]">
            ...
            <div
              ...
              className={cn('border-2 border-dashed rounded-[2.5rem] p-6 text-center mb-6 transition-all', isUploading ? 'border-indigo-200 bg-blue-50 cursor-not-allowed' : 'border-sky-100 hover:border-sky-400 hover:bg-sky-50 cursor-pointer group')}
            >
              {pendingFiles.length === 0 ? (
                <>
                  <div className="text-5xl mb-3 group-hover:scale-110 transition-transform">🚀</div>
                  <p className="font-black text-slate-500 text-sm uppercase tracking-widest">Browse Files</p>
                  <p className="text-[10px] text-slate-400 font-bold mt-1">Click to select one or more files</p>
                </>
              ) : (
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {pendingFiles.map((f, i) => {
                      ...
                      const prev  = isImg ? URL.createObjectURL(f) : null;
                      return (
                        <div key={i}>
                          <div className="aspect-square rounded-2xl border border-slate-100 overflow-hidden bg-slate-50 flex items-center justify-center">
                            {prev ? <img src={prev} alt={f.name} className="w-full h-full object-cover" /> : <span className="text-3xl">{emoF(f.name)}</span>}
                          </div>
            ...
            {/* Deploy button → opens confirmation modal first */}
            <button
              onClick={() => {
                if (pendingFiles.length === 0) return addToast('No files selected', 'error');
              ...
              className="w-full py-5 bg-gradient-to-r from-sky-400 to-blue-500 text-white rounded-[2rem] font-black uppercase tracking-[0.2em] text-sm shadow-[8px_8px_0_0_#38bdf8] hover:shadow-[4px_4px_0_0_#38bdf8] transition-all flex items-center justify-center gap-3 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {isUploading ? <><LoaderCircle size={20} className="animate-spin" /> Uploading...</> : <><Upload size={20} /> Deploy to IPFS</>}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="modal-box relative bg-white border-2 border-white rounded-[3rem] p-8 max-w-sm w-full shadow-[12px_12px_0_0_#7dd3fc]">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setGroupModalOpen(false)} className="w-9 h-9 bg-white rounded-full flex items-center justify-center text-slate-400 hover:bg-red-500 hover:text-white transition-all">✕</button>
            </div>
            <div className="space-y-4">
              ...
              <div className="pt-4 flex gap-3">
                <button onClick={() => setGroupModalOpen(false)} className="flex-1 h-12 bg-white text-slate-500 font-black rounded-2xl text-xs uppercase tracking-widest border border-slate-100">Cancel</button>
                <button onClick={saveGroup} className="flex-1 h-12 bg-sky-500 text-white font-black rounded-2xl shadow-[6px_6px_0_0_#bae6fd] text-xs uppercase tracking-widest hover:bg-sky-400 transition-all">Create</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── Deploy to IPFS Confirmation Modal ────────────────────────────────── */}
      {deployConfirm && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/70 backdrop-blur-sm" onClick={() => setDeployConfirm(false)} />
          <div className="modal-box relative bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-[12px_12px_0_0_#7dd3fc] border-4 border-white">
            <div className="text-center mb-6">
              <div className="text-6xl mb-4">🚀</div>
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Deploy to IPFS?</h3>
              <p className="text-sm font-semibold text-slate-400 mt-2 leading-relaxed">
                You are about to pin{' '}
                <span className="text-sky-500 font-black">{pendingFiles.length} file{pendingFiles.length > 1 ? 's' : ''}</span>{' '}
                to IPFS via Pinata. This action is <span className="text-amber-500 font-black">permanent</span> — files will be publicly accessible on the decentralized web.
              </p>
              {pendingFiles.length > 0 && (
                <div className="mt-4 bg-sky-50 border border-sky-100 rounded-2xl p-3 text-left space-y-1.5">
                  {pendingFiles.slice(0, 3).map((f, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <span className="text-base">{f.type.startsWith('image/') ? '🖼️' : '📁'}</span>
                      <span className="font-mono text-[10px] text-slate-500 truncate flex-1">{f.name}</span>
                      <span className="text-[9px] font-black text-slate-400 flex-shrink-0">{(f.size / 1024).toFixed(1)} KB</span>
                    </div>
                  ))}
                  {pendingFiles.length > 3 && <p className="text-[10px] font-black text-slate-400 text-center pt-1">+{pendingFiles.length - 3} more files</p>}
                </div>
              )}
            </div>
            <div className="flex gap-3">
              <button onClick={() => setDeployConfirm(false)} className="flex-1 h-12 bg-slate-50 border-2 border-slate-100 text-slate-500 font-black rounded-2xl text-xs uppercase tracking-widest hover:bg-slate-100 transition-all">
                Cancel
              </button>
              <button
                onClick={() => { setDeployConfirm(false); handleUpload(); }}
                className="flex-1 h-12 bg-gradient-to-r from-sky-400 to-blue-500 text-white font-black rounded-2xl text-xs uppercase tracking-widest shadow-[4px_4px_0_0_#38bdf8] hover:shadow-none transition-all flex items-center justify-center gap-2"
              >
                🚀 Deploy Now
              </button>
            </div>
      ...
      {/* ── Delete / Unpin Confirmation Modal ────────────────────────────────── */}
      {deleteConfirm && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/70 backdrop-blur-sm" onClick={() => setDeleteConfirm(null)} />
          <div className="modal-box relative bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-[12px_12px_0_0_#fca5a5] border-4 border-white">
            <div className="text-center mb-6">
              <div className="text-6xl mb-4">🗑️</div>
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">
                {deleteConfirm.type === 'file' ? 'Unpin File?' : 'Delete Group?'}
              </h3>
              <p className="text-sm font-semibold text-slate-400 mt-2 leading-relaxed">
                {deleteConfirm.type === 'file' ? (
                  <>
                    You are about to <span className="text-red-500 font-black">unpin</span>{' '}
                    <span className="text-slate-700 font-black break-all">"{deleteConfirm.fileName}"</span>{' '}
                    from Pinata. The CID remains on IPFS but will no longer be actively pinned.
                  </>
                ) : (
                  <>
                    You are about to <span className="text-red-500 font-black">permanently delete</span>{' '}
                    the group <span className="text-slate-700 font-black">"{deleteConfirm.fileName}"</span>.
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 h-12 bg-slate-50 border-2 border-slate-100 text-slate-500 font-black rounded-2xl text-xs uppercase tracking-widest hover:bg-slate-100 transition-all">
                Cancel
              </button>
              <button
                onClick={() => {
                  if (deleteConfirm.type === 'file') deleteFile(deleteConfirm.fileId);
                  else deleteGroup(deleteConfirm.fileId);
                  setDeleteConfirm(null);
                }}
                className="flex-1 h-12 bg-red-500 text-white font-black rounded-2xl text-xs uppercase tracking-widest shadow-[4px_4px_0_0_#fca5a5] hover:bg-red-600 hover:shadow-none transition-all flex items-center justify-center gap-2"
              >
                🗑️ {deleteConfirm.type === 'file' ? 'Unpin' : 'Delete'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-10 right-10 z-[200] flex flex-col gap-3 pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className={cn('pointer-events-auto flex items-center gap-3 px-6 py-4 rounded-[1.5rem] border-2 border-white font-black text-xs uppercase tracking-widest shadow-[6px_6px_0_0_#bae6fd] animate-in slide-in-from-right-4 duration-300',
            t.type === 'success' ? 'bg-green-400 text-black' : t.type === 'error' ? 'bg-red-400 text-white' : 'bg-white text-slate-700')}>
