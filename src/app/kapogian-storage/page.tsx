        ...
        <aside className="sidebar fixed top-20 left-0 h-[calc(100vh-5rem)] w-52 z-40 flex flex-col py-7 px-3 hidden lg:flex">
          ...
          <nav className="flex-1 space-y-1">
            ...
            <button onClick={() => setPage('groups')} className={cn("w-full nav-item flex items-center gap-3 px-4 py-2.5 rounded-2xl font-black text-xs transition-all border-2 border-transparent", page === 'groups' ? "active" : "text-slate-400 hover:bg-amber-50")}>
              ...
              Groups
              <span className="ml-auto bg-sky-100 text-sky-600 text-[10px] font-black px-2 py-0.5 rounded-full">{groups.length}</span>
            </button>
          </nav>
        </aside>
          ...
          <div className="max-w-6xl mx-auto px-5 pt-10 pb-28">
            ...
            <div className="flex items-center justify-between mb-7 flex-wrap gap-4 pl-12 lg:pl-0">
              ...
              <div className="flex gap-2">
                <button onClick={() => { fetchFiles(); fetchGroups(); }} className="squishy bg-white border-2 border-slate-100 rounded-2xl w-12 h-12 flex items-center justify-center text-slate-400 hover:text-sky-500 transition-all shadow-sm">
            ...
            {page === 'files' ? (
                  ...
                  <>
                      ...
                      {currentFiles.map((file) => (
                        <div key={file.id} className="file-card group rounded-[2.5rem] overflow-hidden">
                          <div className={cn("relative h-48 flex items-center justify-center overflow-hidden", thCls(file.name))}>
                            ...
                            <button onClick={() => deleteFile(file.id)} className="absolute top-4 right-4 w-8 h-8 bg-red-500 text-white rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
                              <Trash2 size={14} />
                            </button>
                          </div>
                          <div className="p-5 bg-white space-y-3">
                            ...
                            <button onClick={() => copyText(file.url, "URL")} className="w-full h-9 bg-sky-50 rounded-xl flex items-center justify-center gap-2 border-2 border-sky-100 text-sky-500 font-black text-[10px] uppercase tracking-widest hover:bg-sky-500 hover:text-white transition-all shadow-sm">
                    ...
                    <div className="mt-12 flex justify-center">
                      <div className="neo-sm rounded-2xl px-6 py-3 border-2 border-white flex items-center gap-6">
                        <select value={rowsPerPage} onChange={(e) => { setRowsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="bg-sky-50 border-2 border-sky-100 rounded-xl px-2 py-1 text-xs font-black text-slate-600">
                          <option value={8}>8</option><option value={12}>12</option><option value={24}>24</option>
                        </select>
                        <div className="flex gap-4 items-center">
                          <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="squishy w-10 h-10 neo-sm rounded-xl flex items-center justify-center text-slate-400 hover:text-sky-500 disabled:opacity-20 border border-white">
                            <ArrowLeft size={18} />
                          </button>
                          <span className="text-sm font-black text-slate-700 font-mono">{currentPage} / {totalPages}</span>
                          <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="squishy w-10 h-10 neo-sm rounded-xl flex items-center justify-center text-slate-400 hover:text-sky-500 disabled:opacity-20 border border-white">
                            <ArrowRight size={18} />
                          </button>
                        </div>
                      </div>
              ...
              <div className="animate-in fade-in duration-500">
                {isLoadingGroups ? (
                  ...
                  <div className="text-center py-32 opacity-40">
                    <FolderOpen size={64} className="mx-auto mb-4 text-slate-200" />
                    <p className="font-black uppercase tracking-widest text-slate-400">No Groups Found</p>
                  </div>
                ) : (
                  <div className="bg-white border-4 border-slate-100 rounded-[2.5rem] overflow-hidden shadow-sm">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50/50 border-b-2 border-slate-100 text-[10px] font-black uppercase text-slate-400 tracking-widest">
                          ...
                          <tr key={group.id} className="hover:bg-slate-50 transition-colors group">
                            ...
                            <td className="px-8 py-5">
                              <div onClick={() => copyText(group.id, "Group ID")} className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-1.5 cursor-pointer w-fit">
                                ...
                                <Copy size={12} className="text-slate-200" />
                              </div>
                            </td>
                            <td className="px-8 py-5">
