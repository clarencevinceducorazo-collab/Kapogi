...
export function OrdersPanel({ account }: { account: any }) {
  ...
  const getTrackingUrl = (carrier: string, trackingNumber: string) => {
    ...
    if (c.includes("J&T") || c.includes("SPX") || c.includes("NINJA"))
          ...
          return (
            ...
            >
              ...
              <div className="flex-grow text-center sm:text-left overflow-hidden w-full">
                <p className="font-bold text-slate-800 truncate uppercase text-lg">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <span
                  className={`flex-1 sm:flex-none px-4 py-2 rounded-xl text-xs font-bold border-2 whitespace-nowrap flex items-center justify-center gap-1.5 ${si.bg} ${si.textColor} border-current/10`}
                >
                  {si.icon} {si.text}
                </span>
          ...
          <div className="bg-white border-4 border-black rounded-[2.5rem] p-6 sm:p-8 max-w-lg w-full shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            {selectedOrder.status >= ORDER_STATUS.SHIPPED ? (
              <div className="bg-blue-50 border-4 border-blue-100 p-6 rounded-3xl relative overflow-hidden">
                ...
                <p className="text-xs font-black text-blue-400 uppercase mb-3 tracking-widest">
                  Live Logistics
                </p>
              ...
              <div className="text-center py-10 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                <LoaderCircle
                  className="mx-auto mb-2 text-slate-300 animate-spin"
                  size={24}
                />
                <p className="italic text-slate-400 font-bold text-sm">
                  Character is being summoned into physical form... 🧵
                </p>
              </div>
            )}
