...
export function UserMessageDrawer({ walletAddress }: { walletAddress: string }) {
  ...
  // ── NEW: ref so handleSend can always read latest messages without stale closure
  ...
  // ── NEW helper: call /api/ai-reply after every user message (fire-and-forget)
  // This makes AI work even when the admin tab is closed.
  // The API route publishes the AI reply back via Ably REST, so the user sees it.
  const triggerAI = useCallback((text: string) => {
    // Build snapshot of full conversation including the message just sent
    const history = [
    ...
    fetch("/api/ai-reply", {
      ...
      body: JSON.stringify({ walletAddress, messages: history }),
    }).catch(() => {
      // Silently ignore — admin can still reply manually if AI fails
    });
  }, [walletAddress]);

  // ── Send message (same as original, + triggerAI call at the end) ──────────
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      // Original Ably publish — this is what makes messages work
      ...
      // ── NEW: trigger AI reply in background so it works when admin is offline
      triggerAI(text);
  ...
  }, [input, sending, walletAddress, triggerAI]);

  // ── Quick button handler (same as original, + triggerAI) ──────────────────
  ...
  }, [sending, walletAddress, triggerAI]);
